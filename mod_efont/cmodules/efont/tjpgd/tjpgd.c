...
#define JD_FORMAT     jd->color_format
...
)
{
	...
	for (blk = 0; blk < nby + 2; blk++) {	/* Get nby Y blocks and two C blocks */
		...
		if (cmp && jd->ncomp != 3) {		/* Clear C blocks if not exist (monochrome image) */
			...
			if (JD_FORMAT != JDF_MONO || !cmp) {	/* C components may not be processed if in grayscale output */
...
)
{
	...
	if (!JD_USE_SCALE || jd->scale != 3) {	/* Not for 1/8 scaling */
		...
		if (JD_FORMAT != JDF_MONO) {	/* RGB output (build an RGB MCU from Y/C component) */
			for (iy = 0; iy < my; iy++) {
				pc = py = jd->mcubuf;
			...
			for (iy = 0; iy < my; iy++) {
				...
				for (ix = 0; ix < mx; ix++) {
                    ...
                    /* Get and store a Y value as grayscale [patch01] */
					// *pix++ = (uint8_t)*py++;
                    if (JD_FASTDECODE >= 1) {
                        *pix++ = BYTECLIP(*py++);
                    } else {
		...
		if (JD_USE_SCALE && jd->scale) {
			...
			a = (mx - w) * (JD_FORMAT != JDF_MONO ? 3 : 1);	/* Bytes to skip for next line in the square */
			op = (uint8_t*)jd->workbuf;
			for (iy = 0; iy < my; iy += w) {
				for (ix = 0; ix < mx; ix += w) {
					pix = (uint8_t*)jd->workbuf + (iy * mx + ix) * (JD_FORMAT != JDF_MONO ? 3 : 1);
					r = g = b = 0;
					for (y = 0; y < w; y++) {	/* Accumulate RGB value in the square */
						for (x = 0; x < w; x++) {
							r += *pix++;	/* Accumulate R or Y (monochrome output) */
							if (JD_FORMAT != JDF_MONO) {	/* RGB output? */
								g += *pix++;	/* Accumulate G */
								b += *pix++;	/* Accumulate B */
							}
						}
	...
	if (rx < mx) {	/* Is the MCU spans rigit edge? */
		...
		for (y = 0; y < ry; y++) {
			...
			s += (mx - rx) * (JD_FORMAT != JDF_MONO ? 3 : 1);	/* Skip truncated pixels */
		}
	}

	/* Convert RGB888 to RGB565 if needed */
	if (JD_FORMAT == JDF_RGB565) {
